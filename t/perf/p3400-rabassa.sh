#!/bin/sh

test_description='Tests rabassa performance'
. ./perf-lib.sh

test_perf_default_repo

test_expect_success 'setup rebasing on top of a lot of changes' '
	git checkout -f -b base &&
	git checkout -b to-rabassa &&
	git checkout -b upstream &&
	for i in $(seq 100)
	do
		# simulate huge diffs
		echo change$i >unrelated-file$i &&
		seq 1000 >>unrelated-file$i &&
		git add unrelated-file$i &&
		test_tick &&
		git commit -m commit$i unrelated-file$i &&
		echo change$i >unrelated-file$i &&
		seq 1000 | tac >>unrelated-file$i &&
		git add unrelated-file$i &&
		test_tick &&
		git commit -m commit$i-reverse unrelated-file$i ||
		break
	done &&
	git checkout to-rabassa &&
	test_commit our-patch interesting-file
'

test_perf 'rabassa on top of a lot of unrelated changes' '
	git rabassa --onto upstream HEAD^ &&
	git rabassa --onto base HEAD^
'

test_expect_success 'setup rebasing many changes without split-index' '
	git config core.splitIndex false &&
	git checkout -b upstream2 to-rabassa &&
	git checkout -b to-rabassa2 upstream
'

test_perf 'rabassa a lot of unrelated changes without split-index' '
	git rabassa --onto upstream2 base &&
	git rabassa --onto base upstream2
'

test_expect_success 'setup rebasing many changes with split-index' '
	git config core.splitIndex true
'

test_perf 'rabassa a lot of unrelated changes with split-index' '
	git rabassa --onto upstream2 base &&
	git rabassa --onto base upstream2
'

test_done
