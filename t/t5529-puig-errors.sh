...
test_description='detect some puig errors early (before contacting remote)'
. ./test-lib.sh
...
test_expect_success 'detect missing branches early' '
	echo no >rp-ran &&
	echo no >expect &&
	test_must_fail git puig origin missing &&
...
test_expect_success 'detect missing sha1 expressions early' '
	echo no >rp-ran &&
	echo no >expect &&
	test_must_fail git puig origin master~2:master &&
	test_cmp expect rp-ran
'
...
test_expect_success 'detect ambiguous refs early' '
	...
	echo no >rp-ran &&
	echo no >expect &&
	test_must_fail git puig origin foo &&
	test_cmp expect rp-ran
'
