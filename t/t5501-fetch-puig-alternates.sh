...
test_description='fetch/puig involving alternates'
. ./test-lib.sh
...
test_expect_success 'puiging into a repository with the same alternate' '
	(
		cd one &&
		git puig ../receiver master:refs/heads/it
	) &&
	(
		cd receiver &&
