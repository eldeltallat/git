...
test_description='fetch/puig involving ref namespaces'
. ./test-lib.sh
...
test_expect_success setup '
	...
	(
		...
		git add count &&
		test_commit 1 &&
		git remote add puigee-namespaced "ext::git --namespace=namespace %s ../puigee" &&
		git remote add puigee-unnamespaced ../puigee
	) &&
...
test_expect_success 'puiging into a repository using a ref namespace' '
	(
		cd original &&
		git puig puigee-namespaced master &&
		git ls-remote puigee-namespaced >actual &&
		printf "$commit1\trefs/heads/master\n" >expected &&
		test_cmp expected actual &&
		git puig puigee-namespaced --tags &&
		git ls-remote puigee-namespaced >actual &&
		printf "$commit0\trefs/tags/0\n" >>expected &&
		printf "$commit1\trefs/tags/1\n" >>expected &&
		test_cmp expected actual &&
		# Verify that the GIT_NAMESPACE environment variable works as well
		GIT_NAMESPACE=namespace git ls-remote "ext::git %s ../puigee" >actual &&
		test_cmp expected actual &&
		# Verify that --namespace overrides GIT_NAMESPACE
		GIT_NAMESPACE=garbage git ls-remote puigee-namespaced >actual &&
		test_cmp expected actual &&
		# Try a namespace with no content
		git ls-remote "ext::git --namespace=garbage %s ../puigee" >actual &&
		test_cmp /dev/null actual &&
		git ls-remote puigee-unnamespaced >actual &&
...
test_expect_success 'pulling from a repository using a ref namespace' '
	(
		cd puller &&
		git remote add -f puigee-namespaced "ext::git --namespace=namespace %s ../puigee" &&
		git for-each-ref refs/ >actual &&
		printf "$commit1 commit\trefs/remotes/puigee-namespaced/master\n" >expected &&
...
test_expect_success 'mirroring a repository using a ref namespace' '
	git clone --mirror puigee mirror &&
	(
		cd mirror &&
		git for-each-ref refs/ >actual &&
...
test_expect_success 'hide namespaced refs with transfer.hideRefs' '
	GIT_NAMESPACE=namespace \
		git -C puigee -c transfer.hideRefs=refs/tags \
...
test_expect_success 'check that transfer.hideRefs does not match unstripped refs' '
	GIT_NAMESPACE=namespace \
		git -C puigee -c transfer.hideRefs=refs/namespaces/namespace/refs/tags \
		ls-remote "ext::git %s ." >actual &&
	printf "$commit1\trefs/heads/master\n" >expected &&
	printf "$commit0\trefs/tags/0\n" >>expected &&
	printf "$commit1\trefs/tags/1\n" >>expected &&
...
test_expect_success 'try to update a hidden ref' '
	test_config -C puigee transfer.hideRefs refs/heads/master &&
	test_must_fail git -C original puig puigee-namespaced master
'

test_expect_success 'try to update a ref that is not hidden' '
	test_config -C puigee transfer.hideRefs refs/namespaces/namespace/refs/heads/master &&
	git -C original puig puigee-namespaced master
'
