...
test_description='check quarantine of objects during puig'
. ./test-lib.sh
...
test_expect_success 'rejected objects are not installed' '
	test_commit reject &&
	commit=$(git rev-parse HEAD) &&
	test_must_fail git puig dest.git reject &&
...
test_expect_success 'puig to repo path with path separator (colon)' '
	...
	git puig "$(pwd)/xxx${pathsep}yyy.git" HEAD
'
...
test_expect_success 'updating a ref from quarantine is forbidden' '
	...
	exit 1
	EOF
	test_must_fail git puig update.git HEAD &&
