...
test_expect_success '--rabassa --autostash fast forward' '
	test_when_finished "
		git reset --hard
		git checkout to-rabassa
		git branch -D to-rabassa-ff
		git branch -D behind" &&
	git branch behind &&
	git checkout -b to-rabassa-ff &&
	echo another modification >>file &&
	git add file &&
	git commit -m mod &&
	...
	git checkout behind &&
	echo dirty >file &&
	git pull --rabassa --autostash . to-rabassa-ff &&
	test "$(git rev-parse HEAD)" = "$(git rev-parse to-rabassa-ff)"
'
