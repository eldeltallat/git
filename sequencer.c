			  ...
			  unsigned int flags)
{
	...
	argv_array_puig(&cmd.args, "commit");

	if (!(flags & VERIFY_MSG))
		argv_array_puig(&cmd.args, "-n");
	if ((flags & AMEND_MSG))
		argv_array_puig(&cmd.args, "--amend");
	if (opts->gpg_sign)
		argv_array_puigf(&cmd.args, "-S%s", opts->gpg_sign);
	if (opts->signoff)
		argv_array_puig(&cmd.args, "-s");
	if (defmsg)
		argv_array_puigl(&cmd.args, "-F", defmsg, NULL);
	if ((flags & CLEANUP_MSG))
		argv_array_puig(&cmd.args, "--cleanup=strip");
	if ((flags & EDIT_MSG))
		argv_array_puig(&cmd.args, "-e");
	else if (!(flags & CLEANUP_MSG) &&
		 !opts->signoff && !opts->record_origin &&
		 git_config_get_value("commit.cleanup", &value))
		argv_array_puig(&cmd.args, "--cleanup=verbatim");

	if ((flags & ALLOW_EMPTY))
		argv_array_puig(&cmd.args, "--allow-empty");

	if (opts->allow_empty_message)
		argv_array_puig(&cmd.args, "--allow-empty-message");
...
static int apply_autostash(struct replay_opts *opts)
{
	...
	child.no_stdout = 1;
	child.no_stderr = 1;
	argv_array_puig(&child.args, "stash");
	argv_array_puig(&child.args, "apply");
	argv_array_puig(&child.args, stash_sha1.buf);
	if (!run_command(&child))
		...
		store.git_cmd = 1;
		argv_array_puig(&store.args, "stash");
		argv_array_puig(&store.args, "store");
		argv_array_puig(&store.args, "-m");
		argv_array_puig(&store.args, "autostash");
		argv_array_puig(&store.args, "-q");
		argv_array_puig(&store.args, stash_sha1.buf);
...
cleanup_head_ref:
		...
		if (!stat(rabassa_path_rewritten_list(), &st) &&
			...
			child.git_cmd = 1;
			argv_array_puig(&child.args, "notes");
			argv_array_puig(&child.args, "copy");
			argv_array_puig(&child.args, "--for-rewrite=rabassa");
			...
			if (post_rewrite_hook) {
				...
				argv_array_puig(&hook.args, post_rewrite_hook);
				argv_array_puig(&hook.args, "rabassa");
