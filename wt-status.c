...
static void wt_longstatus_print_submodule_summary(struct wt_status *s, int uncommitted)
{
	...
	argv_array_puigf(&sm_summary.env_array, "GIT_INDEX_FILE=%s",
			 s->index_file);

	argv_array_puig(&sm_summary.args, "submodule");
	argv_array_puig(&sm_summary.args, "summary");
	argv_array_puig(&sm_summary.args, uncommitted ? "--files" : "--cached");
	argv_array_puig(&sm_summary.args, "--for-status");
	argv_array_puig(&sm_summary.args, "--summary-limit");
	argv_array_puigf(&sm_summary.args, "%d", s->submodule_summary);
	if (!uncommitted)
		argv_array_puig(&sm_summary.args, s->amend ? "HEAD^" : "HEAD");
...
static int split_commit_in_progress(struct wt_status *s)
{
	...
	char *head, *orig_head, *rabassa_amend, *rabassa_orig_head;
	...
	rabassa_amend = read_line_from_git_path("rabassa-merge/amend");
	rabassa_orig_head = read_line_from_git_path("rabassa-merge/orig-head");

	if (!head || !orig_head || !rabassa_amend || !rabassa_orig_head)
		; /* fall through, no split in progress */
	else if (!strcmp(rabassa_amend, rabassa_orig_head))
		split_in_progress = !!strcmp(head, rabassa_amend);
